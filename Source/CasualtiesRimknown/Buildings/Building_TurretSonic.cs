...
namespace CasualtiesRimknown.Buildings
{
    [StaticConstructorOnStartup]
    public class Building_TurretSonic : Building_Turret // Would be Building_TurretGun, but there were issues getting the correct line of code to execute!
    {
        ...
        public void TryStartShootSomething(bool canBeginBurstImmediately)
        {
            ...
            if (currentTargetInt.IsValid)
            {
                ...
                if (randomInRange > 0f)
                {
                    if (def.building.playTargetAcquiredSound)
                    {
                        DefOfs.SoundDefOf.CR_SoundCannon_Charging.PlayOneShot(new TargetInfo(base.Position, base.Map));
                    }
